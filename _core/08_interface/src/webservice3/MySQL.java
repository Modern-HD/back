...
public class MySQL implements DataBase {
	...
	public boolean insert(String email) {
		System.out.println("MySQL > 회원 정보 입력 시도");
		if (email.length() > 2) {
			if (!searchMember(email)) {
				int from = searchEmptyIdx();
				if (from > 9) {
					System.out.println("MySQL > 회원 정보 입력 실패");
		...
		System.out.println("MySQL > 회원 정보 입력 성공");
		return true;
	}

	@Override
	public boolean select(String email) {
		System.out.println("MySQL > 회원 정보 조회 시도");
		if (searchMember(email)) {
			int from = searchMemberIdx(email);
			System.out.println("비밀번호를 입력해주세요. > ");
			String pwd = sc.next();
			if (member[from].getPwd().equals(pwd)) {
				System.out.println("MySQL > 회원 정보 조회 성공");
			} else {
				System.out.println("MySQL > 회원 정보 조회 실패");
	...
	public boolean update(String email, int update, String set) {
		System.out.println("MySQL > 회원 정보 수정 시도");
		int from = searchMemberIdx(email);
		switch (update) {
		case 1:
		...
		case 3:
			...
			try {
				...
				System.out.println("MySQL > 회원 정보 수정 실패");
	...
	public boolean delete(String email, String pwd) {
		...
		System.out.println("MySQL > 회원 정보 삭제 완료");
		return true;
	}
